...
import { View, Alert, Linking , StyleSheet, Image, ImageBackground, Text, TouchableOpacity} from "react-native";
...
const Therapists = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (status !== "granted") {
        Alert.alert("Permission Denied", "Location access is required to find nearby therapists.");
        return;
      }
  ...
  const fetchNearbyTherapists = async (lat, lng) => {
    try {
      ...
      [out:json];
      (
        node(around:2000,${lat},${lng})["healthcare"];
      ...
      if (data.elements) {
        setTherapists(
          data.elements.map((item) => ({
            id: item.id,
            name: item.tags.name || "Therapist",
            lat: item.lat,
            lng: item.lon,
            address: item.tags["addr:full"] || "No address available",
            googleUrl: `https://www.google.com/search?q=${encodeURIComponent(item.tags.name || "therapist")}`,
          }))
        );
